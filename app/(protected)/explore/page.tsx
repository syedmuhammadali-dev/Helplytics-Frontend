      ...
      <main className="flex-grow container mx-auto px-6 py-12">
        <div className="flex flex-col gap-12">
          ...
          >
            ...
            <h1 className="text-5xl font-bold mb-4 max-w-4xl">Browse help requests with filterable community context.</h1>
            <p className="text-white/60 font-medium text-lg leading-relaxed max-w-3xl">Filter by category, urgency, skills, and location to surface the best matches.</p>
          </motion.div>
          ...
          <div className="grid lg:grid-cols-4 gap-8">
            ...
            <div className="lg:col-span-1 flex flex-col gap-6">
              <div className="premium-card bg-bg-card border-none shadow-none sticky top-32 p-8">
                ...
                <h2 className="text-4xl font-bold text-dark mb-10 tracking-tight">Refine feed</h2>
                ...
                <form className="flex flex-col gap-8">
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="relative">
                      ...
                      <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 text-dark/40 pointer-events-none" size={18} />
                    </div>
                  </div>
                ...
                {feedRequests.map((req, i) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <Link href={`/requests/${i}`} className="px-6 py-2.5 rounded-2xl bg-bg-card text-xs font-bold text-dark hover:bg-dark hover:text-white transition-all">Open details</Link>
                    </div>
                    
                    <div className="flex-grow">
                      <h3 className="font-bold text-2xl mb-3 group-hover:text-primary transition-colors">{req.title}</h3>
                      <p className="text-text-muted leading-relaxed mb-6 line-clamp-2">{req.desc}</p>
                      
                      {req.tags && (
                        <div className="flex flex-wrap gap-2">
                          {req.tags.map(tag => (
                            <span key={tag} className="px-3 py-1.5 rounded-xl bg-bg-card text-[11px] font-bold text-text-muted uppercase tracking-wider">{tag}</span>
                          ))}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="pt-8 border-t border-black/5 flex items-center justify-between">
                      <div>
                        <div className="font-bold text-dark text-lg leading-tight">{req.author}</div>
                        <div className="text-sm text-text-muted">{req.location}</div>
                      </div>
                    </div>
                  </motion.div>
                ))}
