      ...
      <main className="flex-grow container mx-auto px-6 py-12">
        <div className="flex flex-col gap-12">
          ...
          >
            ...
            <span className="section-label text-primary">Interaction / Messaging</span>
            <h1 className="text-5xl font-bold mb-4 max-w-3xl">Keep support moving through direct communication.</h1>
            <p className="text-white/60 font-medium text-lg leading-relaxed">Basic messaging gives helpers and requesters a clear follow-up path once a match happens.</p>
          </motion.div>
          ...
          <div className="grid lg:grid-cols-5 gap-8">
            ...
            <div className="lg:col-span-3 flex flex-col gap-6">
              <div className="premium-card h-full">
                <span className="section-label text-text-muted">Conversation Stream</span>
                <h2 className="text-3xl font-bold text-dark mb-10">Recent messages</h2>
                    ...
                    <div key={i} className="p-8 rounded-[2rem] bg-bg-card border border-black/5 hover:bg-white hover:shadow-xl transition-all cursor-pointer group">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-dark group-hover:text-primary transition-colors">{msg.from}</span>
                          <span className="text-text-muted">→</span>
                          <span className="font-bold text-dark">{msg.to}</span>
                        </div>
                        <div className="px-4 py-1.5 rounded-full bg-white text-[10px] font-bold text-primary border border-primary/10 shadow-sm">{msg.time}</div>
                      </div>
                      <p className="text-text-muted text-sm leading-relaxed group-hover:text-dark transition-colors">{msg.text}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="lg:col-span-2 flex flex-col gap-6">
              <div className="premium-card h-full bg-bg-card border-none shadow-none">
                ...
                <h2 className="text-3xl font-bold text-dark mb-10">Start a conversation</h2>
                ...
                <form className="flex flex-col gap-8">
                  ...
                  <button type="button" className="btn-primary w-full justify-center py-5 text-base mt-4 shadow-lg shadow-primary/20">
                    <Send size={18} />
                    Send message
                  </button>
                </form>
